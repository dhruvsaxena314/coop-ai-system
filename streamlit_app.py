...
with tab4:
    ...
    try:
        ...
        st.info("Make sure networkx and pyvis are installed.")
    ...
    if node_name:
        connections = kg.get_connections(node_name)
        if connections:
    ...
    # Node type filter
    node_types = list(set(nx.get_node_attributes(kg.graph, 'type').values())) if kg.graph.nodes else []
    if node_types:
